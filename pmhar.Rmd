...
# Set types for training data and hold out data(harTest).
...
# Summarize missing data for training and hold out set. 
...
# Remove NAs from hold out dataset
...
testSet <-  harTestTr[-highCorr] # Test set of hold out data/validation set.
dim(harAct)
str(testSet)
```
```{r zeroVar}
...
humArrTest <- harAct[ -indexTrain, ] # Test set/hold back set.
trainVar  <- humArrTrain[,-33]; trainClass <- humArrTrain$classe
```
```{r foldSel}
# Create fold selection for cross validation.
foldIdx <- createFolds(trainClass, returnTrain = TRUE)
# Train control set to 3 repetitions repeated cross validation.
cvCtrl <- trainControl(method = "repeatedcv", index = foldIdx, classProbs = TRUE, savePredictions = TRUE, 
                       repeats = 3) 
```
...
```
Out of sample error for each model: SVM, CART, RandomForest, KNN, NN; 0.742, 0.7461, 0.9949, 1, 1, respectively. 
```{r testSet,echo=TRUE,cache=TRUE}
# Prediction on unseen data using RandomForest.  This model was chosen based upon accuracy and 
# parameter settings. 
testPred <- predict(rfT, testSet[,-33])
str(testPred)
testPred
```
References:
